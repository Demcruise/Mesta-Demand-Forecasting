...
function validateSchedule(input: ScheduleInput) {
  if (input.name.trim().length < 3) throw new ApiError("Name the schedule (at least 3 characters).", "validation");
  if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(input.time)) throw new ApiError("Enter the time as HH:MM (24-hour).", "validation");
  if (input.horizonDays < 1 || input.horizonDays > 180) throw new ApiError("The horizon must be between 1 and 180 days.", "validation");
}
...
export function saveSchedule(ctx: ApiContext, id: string | null, input: ScheduleInput) {
  return write(ctx, "forecast.run.create", () => {
    ...
    if (id) {
      ...
      if (!s) throw new ApiError("Schedule not found.", "not_found");
      const prev = `${s.enabled ? "enabled" : "paused"}${s.autoPublish ? ", auto-publish" : ""}`;
      Object.assign(s, input);
      audit(db, { actorId: ctx.userId, action: "change_settings", entityType: "settings", entityId: s.id, entityLabel: `Schedule · ${s.name}`, previousState: prev, newState: `${s.enabled ? "enabled" : "paused"}${s.autoPublish ? ", auto-publish" : ""}`, reason: null, source: "web" });
      return s;
    }
    ...
    audit(db, { actorId: ctx.userId, action: "change_settings", entityType: "settings", entityId: s.id, entityLabel: `Schedule · ${s.name}`, previousState: null, newState: "created", reason: null, source: "web" });
    return s;
  });
}
...
export function setScheduleEnabled(ctx: ApiContext, id: string, enabled: boolean) {
  return write(ctx, "forecast.run.create", () => {
    ...
    if (!s) throw new ApiError("Schedule not found.", "not_found");
    s.enabled = enabled;
    audit(db, { actorId: ctx.userId, action: "change_settings", entityType: "settings", entityId: s.id, entityLabel: `Schedule · ${s.name}`, previousState: enabled ? "paused" : "enabled", newState: enabled ? "enabled" : "paused", reason: null, source: "web" });
    return s;
  });
}
...
export function deleteSchedule(ctx: ApiContext, id: string) {
  return write(ctx, "forecast.run.create", () => {
    ...
    audit(db, { actorId: ctx.userId, action: "change_settings", entityType: "settings", entityId: id, entityLabel: `Schedule · ${s.name}`, previousState: "active", newState: "deleted", reason: null, source: "web" });
    return true;
  });
}
...
export function createApiKey(ctx: ApiContext, input: { name: string; scopes: ApiScope[]; expiresInDays: number | null }) {
  return write(ctx, "api.manage", () => {
    const db = getDb(ctx.workspaceId);
    if (input.name.trim().length < 3) throw new ApiError("Name the key after the system that will use it.", "validation");
    if (input.scopes.length === 0) throw new ApiError("Choose at least one scope.", "validation");
    ...
    audit(db, { actorId: ctx.userId, action: "change_permissions", entityType: "settings", entityId: key.id, entityLabel: `API key · ${key.name}`, previousState: null, newState: `created (${key.scopes.join(", ")})`, reason: null, source: "web" });
...
export function rotateApiKey(ctx: ApiContext, id: string) {
  return write(ctx, "api.manage", () => {
    ...
    if (!key) throw new ApiError("API key not found.", "not_found");
    if (key.status !== "active") throw new ApiError("Only active keys can be rotated.", "conflict");
    const value = `mdf_live_${secret()}`;
    const old = key.prefix;
    key.prefix = value.slice(0, 13);
    key.createdAt = iso(Date.now());
    key.lastUsedAt = null;
    audit(db, { actorId: ctx.userId, action: "change_permissions", entityType: "settings", entityId: key.id, entityLabel: `API key · ${key.name}`, previousState: `${old}…`, newState: `${key.prefix}… (rotated)`, reason: null, source: "web" });
    return { key, secret: value };
  });
}
...
export function revokeApiKey(ctx: ApiContext, id: string) {
  return write(ctx, "api.manage", () => {
    ...
    key.status = "revoked";
    audit(db, { actorId: ctx.userId, action: "change_permissions", entityType: "settings", entityId: key.id, entityLabel: `API key · ${key.name}`, previousState: prev, newState: "revoked", reason: null, source: "web" });
    return key;
  });
}
...
export function createWebhook(ctx: ApiContext, input: { url: string; description: string; events: WebhookEvent[] }) {
  return write(ctx, "api.manage", () => {
    ...
    try {
      ...
      throw new ApiError("Enter a full URL, for example https://example.com/hooks/mesta.", "validation");
    }
    if (url.protocol !== "https:") throw new ApiError("Webhook endpoints must use HTTPS.", "validation");
    if (input.events.length === 0) throw new ApiError("Choose at least one event.", "validation");
    ...
    audit(db, { actorId: ctx.userId, action: "integration_update", entityType: "settings", entityId: hook.id, entityLabel: `Webhook · ${url.host}`, previousState: null, newState: `created (${hook.events.length} events)`, reason: null, source: "web" });
    return hook;
  });
}
...
export function setWebhookEnabled(ctx: ApiContext, id: string, enabled: boolean) {
  return write(ctx, "api.manage", () => {
    ...
    if (!hook) throw new ApiError("Webhook not found.", "not_found");
...
export function saveView(ctx: ApiContext, input: { name: string; surface: SavedView["surface"]; query: string; shared: boolean }) {
  return write(ctx, null, () => {
    const db = getDb(ctx.workspaceId);
    if (input.name.trim().length < 2) throw new ApiError("Name the view.", "validation");
    if (db.platform.savedViews.some((v) => v.surface === input.surface && v.ownerId === ctx.userId && v.name.toLowerCase() === input.name.trim().toLowerCase())) {
      throw new ApiError("You already have a view with this name.", "conflict");
    }
...
export function deleteView(ctx: ApiContext, id: string) {
  return write(ctx, null, () => {
    ...
    if (!view) throw new ApiError("View not found.", "not_found");
    if (view.ownerId !== ctx.userId) throw new ApiError("Only the owner can delete a view.", "permission");
...
export function updateNotificationRules(ctx: ApiContext, rules: NotificationRules) {
  return write(ctx, null, () => {
    ...
    if (rules.quietHours.enabled && (!hhmm.test(rules.quietHours.start) || !hhmm.test(rules.quietHours.end))) {
      throw new ApiError("Enter quiet hours as HH:MM (24-hour).", "validation");
    }
