    ...
    <div className="flex min-h-dvh items-start justify-center bg-[#eef0f3] px-4 pt-[10vh] text-[#1d2330] dark:bg-[#11151b] dark:text-[#e6e9ee]">
      <div className="w-full max-w-md">
        <div className="mb-4 flex items-center gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-200" role="note">
          ...
          Demo identity provider. Your real IdP replaces this screen.
        </div>
        <div className="rounded-lg border border-black/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-[#1a2029]">
          ...
          {phase === "choose" && (
            <>
              <h1 className="mt-2 text-xl font-bold">Choose an account</h1>
              <p className="mt-1 text-sm text-[#5b6472] dark:text-[#9aa3b0]">to continue to Mesta Demand Forecasting</p>
              <ul className="mt-5 flex flex-col gap-1.5" role="radiogroup" aria-label="Demo accounts">
                {identities.map((u) => (
                  <li key={u.id}>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs font-semibold text-[#5b6472] dark:text-[#9aa3b0]">
                        {u.status === "suspended" ? "Disabled" : ROLE_LABELS[u.role]}
                      </span>
                    </button>
                  </li>
                ))}
              ...
              <Button variant="primary" size="lg" className="mt-5 w-full" disabled={!selected} onClick={() => setPhase("mfa")}>
                Sign in as {findUser(selected)?.name.split(" ")[0] ?? "selected account"}
              </Button>
            </>
          )}
          {(phase === "mfa" || phase === "verifying") && (
            <>
              <h1 className="mt-2 text-xl font-bold">Verify it&apos;s you</h1>
              <p className="mt-1 text-sm text-[#5b6472] dark:text-[#9aa3b0]">Approve the sign-in request in your authenticator app.</p>
              <div className="mt-6 flex flex-col items-center gap-3 rounded-md border border-black/10 p-6 dark:border-white/10">
                ...
                <p className="text-center text-sm">
                  Request sent to <strong>{findUser(selected)?.name}</strong>&apos;s registered device.
                </p>
                <p className="text-xs text-[#5b6472] dark:text-[#9aa3b0]">Number to match: 42</p>
              </div>
              <Button variant="primary" size="lg" className="mt-5 w-full" onClick={approve} loading={phase === "verifying"} loadingText="Verifying">
