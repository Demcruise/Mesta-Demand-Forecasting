...
| Page-level copy — **P1 in progress** | Done: the whole Model section (FE-P1-001…004) and the whole Scenarios section (FE-P1-005…007) — Skenario list, builder, detail, Bandingkan Skenario and the impact preview — plus shared metric definitions and scenario driver labels/help. Remaining: Planning / Approval / Monitoring / Notifications (FE-P1-008…013), Governance (FE-P1-014…019), Insights, Onboarding, Schedules. Route `metadata.title` values are also still English, as are the seeded exception reasons, scenario names/rationales and model limitation strings in the mock data. This is why the UI remains partly mixed-language. |
